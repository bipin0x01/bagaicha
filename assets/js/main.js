...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (cartModal && cartOpenBtn && cartCloseBtn) {
    ...
    cartOpenBtn.addEventListener("click", () => {
      ...
      updateCartUI(); // Refresh UI on open
    });
  ...
  // Attach safe click event handlers for "Add to Cart" and "Buy Now" on product grids
  // Supports both .product-grid-item (homepage/product page) and .product-card (shop page)
  document.body.addEventListener("click", (e) => {
    ...
    if (shopCard) {
      ...
      const price = parseFloat(priceEl.textContent.replace(/Rs\./i, "").replace(/[^\d.]/g, ""));
      const imageUrl = imgEl.getAttribute("src");
      const id    = name.toLowerCase().replace(/[^a-z0-9]/g, "-");

      e.preventDefault();

      if (btn.classList.contains("btn-overlay-cart") || btn.textContent.includes("Add to Cart")) {
        addToCart(id, name, price, imageUrl, 1);
        showToast("Added to Cart", `<strong>${name}</strong> has been added to your cart!`, "success");
      } else {
        addToCart(id, name, price, imageUrl, 1);
    ...
    // ── Legacy product-grid-item (homepage / product detail page) ─────────
    const productCard = btn.closest(".product-grid-item");
    if (!productCard) return;

    const titleElement = productCard.querySelector(".product-grid-item-info h2");
    const priceElement = productCard.querySelector(".product-grid-item-info .price h4");
    const imgElement   = productCard.querySelector(".product-grid-item-img img");

    if (!titleElement || !priceElement || !imgElement) return;

    const name  = titleElement.textContent.trim();
    const rawPriceText = priceElement.textContent.trim();
    const price = parseFloat(rawPriceText.replace(/Rs\./i, "").replace(/[^\d.]/g, ""));
    ...
    if (btn.textContent.includes("Add to Cart") || btn.id === "addCart") {
