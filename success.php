...
if (isset($_GET['method']) && $_GET['method'] === 'cod' && isset($_GET['uuid'])) {
    ...
    if ($response && isset($response['status']) && $response['status'] === 'COMPLETE') {
        ...
        $returned_signature = $response['signature'];
        
        // Reconstruct HMAC-SHA256 signature to verify integrity
        ...
        $expected_signature = base64_encode($hmac_hash);

        // Security check using constant-time comparison
        if (hash_equals($expected_signature, $returned_signature)) {
