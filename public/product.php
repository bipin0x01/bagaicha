            ...
            <div class="product-grid-item-info flex flex-col justify-between py-2">
                ...
                <div>
                    ...
                    <div class="product-grid-item-add flex gap-4 w-full">
                        <button class="btn-buy flex-1 py-3.5 text-center text-xs font-bold border-2 border-primary hover:bg-primary-light/35 text-primary rounded-xl transition-all cursor-pointer" id="addCart" type="button"
                            data-id="<?php echo $product['id']; ?>"
                            data-name="<?php echo htmlspecialchars($product['name'], ENT_QUOTES, 'UTF-8'); ?>"
                            data-price="<?php echo htmlspecialchars($product['price']); ?>"
                            data-img="<?php echo htmlspecialchars($product['image_url'], ENT_QUOTES, 'UTF-8'); ?>">Add to Cart</button>
                        <button class="btn-buy flex-1 py-3.5 text-center text-xs font-bold bg-primary hover:bg-primary-dark text-white rounded-xl transition-all shadow-md hover:shadow-primary/30 cursor-pointer" type="button"
                            data-id="<?php echo $product['id']; ?>"
                            data-name="<?php echo htmlspecialchars($product['name'], ENT_QUOTES, 'UTF-8'); ?>"
                            data-price="<?php echo htmlspecialchars($product['price']); ?>"
                            data-img="<?php echo htmlspecialchars($product['image_url'], ENT_QUOTES, 'UTF-8'); ?>">Buy Now</button>
                    </div>
                </div>
            </div>
                    ...
                    ?>
                        <div class="product-card group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden"
                            ...
                            <div class="grid grid-cols-2 border-t border-gray-100 bg-gray-50/50 divide-x divide-gray-100">
                                <button class="btn-buy py-2.5 text-center text-[10px] font-semibold text-gray-600 hover:text-primary hover:bg-purple-50/50 transition-colors cursor-pointer" type="button"
                                    data-id="<?php echo $p['id']; ?>"
                                ...
                                <button class="btn-buy py-2.5 text-center text-[10px] font-semibold text-gray-700 hover:text-white hover:bg-primary transition-colors cursor-pointer" type="button"
                                    data-id="<?php echo $p['id']; ?>"
                                    data-name="<?php echo htmlspecialchars($p['name'], ENT_QUOTES, 'UTF-8'); ?>"
                                    data-price="<?php echo htmlspecialchars($p['price']); ?>"
