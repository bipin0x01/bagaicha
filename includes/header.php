...
<nav class="navbar sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm flex items-center justify-between px-6 md:px-12 h-20">
    ...
    <div class="user-btn relative flex items-center gap-3">
        ...
        <?php if (!empty($email)): ?>
            <button type="button" class="user-btn px-3 py-2 bg-gray-50 hover:bg-purple-50 text-gray-700 hover:text-primary rounded-xl transition-all border border-gray-100 flex items-center gap-2 cursor-pointer text-xs font-semibold max-w-[200px]" id="user-btn" data-dropdown="true">
            ...
            <div id="user-menu" class="hidden absolute right-0 top-[calc(100%+10px)] w-64 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden z-50">
                <div class="px-4 py-4 border-b border-gray-100 bg-gray-50/50">
                    <p class="text-[11px] uppercase tracking-wide text-gray-400 font-semibold">Signed in as</p>
        ...
        <?php else: ?>
            <button type="button" class="user-btn px-4 py-2.5 bg-primary hover:bg-primary-dark text-white rounded-xl transition-all border border-primary shadow-sm hover:shadow-md flex items-center justify-center relative cursor-pointer text-xs font-bold" id="user-btn" data-target="<?php echo htmlspecialchars($account_target); ?>">
